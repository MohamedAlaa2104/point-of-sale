@can('edit services')
<a class="link-icon-edit" href="{{route('dashboard.services.edit',$row->id)}}"><i class="fe fe-edit"></i></a>
@endcan
@can('delete services')
...
<div class="modal" id="modaldemo{{$row->id}}">
    ...
    <form id="delete-form-{{$row->id}}" action="{{route('dashboard.services.destroy', $row->id)}}" method="POST">
        @method('DELETE')
        @csrf
    </form>
</div>
