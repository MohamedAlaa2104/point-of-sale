<a class="link-icon-edit" href="{{route('dashboard.service.env.show',$row->id)}}"><i class="fe fe-eye"></i></a>
...
<div class="modal" id="modaldemo{{$row->id}}">
    ...
    <form id="delete-form-{{$row->id}}" action="{{route('dashboard.service.env.destroy', $row->id)}}" method="POST">
        @method('DELETE')
        @csrf
    </form>
</div>
