...
<a class="link-icon-edit"  href="{{route('dashboard.articles.edit', $row->id)}}"><i class="fe fe-edit"></i></a>
...
<div class="modal" id="modaldemo{{$row->id}}">
    ...
    <form id="delete-form-{{$row->id}}" action="{{route('dashboard.articles.destroy', $row->id)}}" method="POST">
        @method('DELETE')
        @csrf
    </form>
</div>
