    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <div>
                        <h6 class="card-title mb-1">@lang('dashboard.edit_role')</h6>
                        <p class="text-muted card-sub-title"></p>
                    </div>
                    <form method="POST" action="{{route('dashboard.roles.update', $role->id)}}" class="form-horizontal">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <p class="mg-b-10">@lang('dashboard.name')</p>
                            <input type="text" value="{{$role->name}}" name="name" class="form-control" id="inputName">
                        </div>
                        <div class=" mg-b-20 mg-lg-b-0">
                            ...
                            <select name="permissions[]" multiple="multiple" class="testselect2">
                                @foreach($permissions as $id=>$name)
                                    <option value="{{$id}}" {{$role->hasPermissionTo($name) ? 'selected' : ''}}>
                                        {{$name}}
                                    </option>
                                @endforeach
                            </select>
