<a class="link-icon-show"  href="{{route('dashboard.applicants.show', $row->id)}}"><i class="fe fe-eye"></i></a>
...
<div class="modal" id="modaldemo{{$row->id}}">
    ...
    <form id="delete-form-{{$row->id}}" action="{{route('dashboard.applicants.destroy', $row->id)}}" method="POST">
        @method('DELETE')
        @csrf
    </form>
</div>
