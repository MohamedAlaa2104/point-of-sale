<a class="link-icon-show"  href="{{route('dashboard.jobs.show', $row->id)}}"><i class="fe fe-eye"></i></a>

<a class="link-icon-edit"  href="{{route('dashboard.jobs.edit', $row->id)}}"><i class="fe fe-edit"></i></a>
...
<div class="modal" id="modaldemo{{$row->id}}">
    ...
    <form id="delete-form-{{$row->id}}" action="{{route('dashboard.jobs.destroy', $row->id)}}" method="POST">
        @method('DELETE')
        @csrf
    </form>
</div>
